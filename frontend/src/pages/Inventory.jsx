...
const Inventory = () => {
  ...
  const handleDeleteBag = async (id) => {
    if (window.confirm('Remove this blood bag record from inventory?')) {
      try {
        await api.delete(`/inventory/${id}`);
        fetchInventory();
      } catch {
        alert('Failed to remove bag. It may be linked to an issued request.');
      }
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <PageHeader
        ...
        subtitle="Track available blood units and freezer storage"
      ...
      <div className="card">
        <div className="flex-between" style={{ marginBottom: '14px' }}>
          <div>
            ...
            <p className="card-section-subtitle">Real-time whole blood inventory by group</p>
          </div>
                  ...
                  {paged.length === 0 ? (
                    ...
                    paged.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td>
                          {item.status === 'Expired' && (
                            <button type="button" onClick={() => handleDeleteBag(item.id)} className="icon-btn danger" title="Remove expired bag">
