...
const ReportView = ({ type }) => {
  ...
  const buildReport = (data) => {
    ...
    if (type === 'requests') {
      ...
      data.forEach((r) => {
        ...
        const matchHospital = requestHospital === 'all' || r.hospital?.id === Number(requestHospital);
        ...
        const reqIdStr = r.id ? String(r.id) : '';
        ...
        if (!r.items?.length) {
          rows.push([reqIdStr, r.hospital?.hospitalName || '', r.requestDate || '', r.priority || '', r.status || '', '', '', '', r.requestedBy || '']);
        }
      });
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.message || err.message || 'Failed to generate report preview.';
